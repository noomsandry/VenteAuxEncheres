...
use VendeurBundle\Entity\Magasin;
use CompteBundle\Entity\Compte;
...
class RestRegistrationController extends FOSRestController implements ClassResourceInterface
{
    ...
    public function registerAction(Request $request)
    {
        ...
        if (!$form->isValid() || ($user->getType()->getId() == 2 && $user->getMagasin() == null)) {
        ...
        if($user->getType()->getId() == 1)
            $user->addRole('ROLE_ACHETEUR');
        else if($user->getType()->getId() == 2){
            $user->addRole('ROLE_VENDEUR');
            if($user->getMagasin() != null){
                $magasin = new Magasin();
                $magasin->setNom($user->getMagasin());
                $magasin->setVendeur($user);
                $em->persist($magasin);
            }
        }else
